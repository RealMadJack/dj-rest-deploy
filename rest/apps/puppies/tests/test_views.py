...
class GetSinglePuppy(TestCase):
    ...
    def setUp(self):
        self.diana = Puppy.objects.create(name='Diana', age=1, breed='Pug', color='Apricot')
        self.beth = Puppy.objects.create(name='Elizabeth', age=3, breed='Beagle', color='Orange Black')
    ...
    def test_get_valid_single_puppy(self):
        response = client.get(reverse('puppies:get_delete_update_puppy', kwargs={'pk': self.diana.pk}))
        puppy = Puppy.objects.get(pk=self.diana.pk)
        serializer = PuppySerializer(puppy)
        self.assertEqual(response.data, serializer.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_invalid_single_puppy(self):
        response = client.get(reverse('puppies:get_delete_update_puppy', kwargs={'pk': 30}))
...
class UpdateSinglePuppyTest(TestCase):
    ...
    def test_valid_update_puppy(self):
        response = client.put(
            reverse('puppies:get_delete_update_puppy', kwargs={'pk': self.beth.pk}),
            data=json.dumps(self.valid_payload),
            content_type='application/json',
        )
...
class DeleteSinglePuppyTest(TestCase):
    ...
    def test_invalid_delete_puppy(self):
        response = client.delete(reverse('puppies:get_delete_update_puppy', kwargs={'pk': 30}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
